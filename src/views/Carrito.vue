<template>
  <div>
    <v-container>
      ...
      <h2 class="my-10" v-if="this.carrito[0] == undefined">
        Tu carrito está vacío, revisa nuestro catálogo y agrega tu amigurumi
        favorito!
      </h2>
      ...
      <v-form
        ...
        ref="form"
        v-if="this.carrito[0] != undefined"
      >
        <v-card class="mx-auto p-5" tile>
          <v-card-title class="p-1"
            >Ingresa tus datos para ponernos en contacto!</v-card-title
          >
          <v-text-field
            class="inputNombre"
            v-model="nombre"
          ...
          <v-text-field
            v-on:keyup.enter="enviarCarrito()"
            class="inputTelefono"
            v-model="telefono"
            label="Teléfono"
            required
